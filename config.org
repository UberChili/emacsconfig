...
;; Deleted this to just try maybe Jetbrains?
  ;; (setq doom-font (font-spec :family "Maple Mono NF"
  ;;                           ;; I think size 16 is comfy for macbook, use a little bigger on Desktop
  ...
  ;;                                          ;; 16 for macbook
  ;;                                          :size 16 )
  ;;      doom-big-font (font-spec :family "Maple Mono Bold NF"
  ;;                               :size 25))

  (setq doom-font (font-spec :family "JetBrainsMono Nerd Font"
                            ;; I think size 16 is comfy for macbook, use a little bigger on Desktop
                            :size 16 :weight 'semi-light)
       ;; Recently, I decided to increase the font size by default on Linux, it was at 18 before, I'll try how it feels at 20 for some days
...
But I think I'll try gruber darker for a while:
#+begin_src emacs-lisp
...
** Using Circadian
Not really interested in using Circadian anymore. However I'll keep it commented for now. Remember to add again in packages if we want to try again.
#+begin_src emacs-lisp
...
** Setting only one theme
I'm just using Everforest for now. I think it looks neat and uses that big font on Org files' titles. Sometimes I still think Nord (and also Nord light) still look great so we might go back to that
#+begin_src emacs-lisp
...
* LSP
** Clangd
Same shit as with Odin... Will comment out the first block which was "working" with lsp-mode and will try out Eglot in the second uncommented block. Let's see what happens.
#+begin_src emacs-lisp
...
;; ; This makes save on format use clang-format and acts accordingly to the .clang-format file on
...
** Ols (Odin)
I include here what is/was currently working, *BEFORE* trying Eglot out. That is, what is working for Odin as an lsp-mode user.
Then, the next block will be what I will try with Eglot.

I recently deleted a bunch of commented-out code that I didn't even needed anymore for a decent amount of weeks. Hopefully I won't need it again. Just in the same fashion, I will likely delete this line later.
#+begin_src emacs-lisp
(use-package! odin-mode
  :defer t
  :mode ("\\.odin\\'" . odin-mode)
  :config
  (add-hook 'odin-mode-local-vars-hook #'lsp! 'append)
...
;;   :mode ("\\.odin\\'" . odin-mode)
;;   :config
;;   (add-hook 'odin-mode-local-vars-hook #'eglot-ensure 'append)
;;   (add-hook 'odin-ts-mode-local-vars-hook #'eglot-ensure 'append))
#+end_src
...
#+end_src
** clang-format
I was using the following code as a quick fix when learning through LearnOpenGL as I needed the LSP/format to not automatically sort includes on save. Then I learned more about all of this and using a .clang-format file. I'll keep this in here just for reference but I might delete soon.
#+begin_src emacs-lisp
;; (after! format
...
This is the correct code for now:
#+begin_src emacs-lisp
  ;; Set clang-format options
(after! format
  ;; Set clang-format options
...
* Org and Org-Roam
Need to be very careful with this if I'll be playing around with md-roam. I better coment out this entire next src block and add a new one. Remember, this worked correctly.
#+begin_src emacs-lisp
;; ------------------------------
...
(after! org-roam
  ...
  (setq org-roam-index-file "~/Nextcloud/Notes/inbox.org")
  
  ;; Override the configuration for the default filename of new nodes
  ;; (Now it doesn't use the fucking full id on filename)
  (setq org-roam-capture-templates
        ...
        ("s" "source material" plain
         "%?"
         :if-new (file+head "source-material/${slug}.org"
                            "#+title: ${title}\n#+filetags: :source:\n")
         :unnarrowed t)))
  ...
  ;; Display behavior of the org-roam buffer
...
;;       '("~/Nextcloud/Notes/"
;;         "~/Nextcloud/Notes/daily/"))
(setq org-agenda-files
      (directory-files-recursively "~/Nextcloud/Notes/" "\\.org$"))
#+end_src
...
* Org-mode capture templates (Copied from Joshua Blais. Thanks)
#+begin_src emacs-lisp
(after! org
(setq org-capture-templates
      '(("t" "Todo" entry
         (file+headline "~/Nextcloud/Notes/inbox.org" "Inbox")
         "* TODO %^{Task}\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:END:\n%?")
        ("e" "Event" entry
         (file+headline "~/Nextcloud/Notes/calendar.org" "Events")
         "* %^{Event}\n%^{SCHEDULED}T\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:CONTACT: %(org-capture-ref-link \"~/Nextcloud/Notes/contacts.org\")\n:END:\n%?")
        ("d" "Deadline" entry
         (file+headline "~/Nextcloud/Notes/calendar.org" "Deadlines")
         "* TODO %^{Task}\nDEADLINE: %^{Deadline}T\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:END:\n%?")
        ("p" "Project" entry
         (file+headline "~/Nextcloud/Notes/projects.org" "Projects")
         "* PROJ %^{Project name}\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:END:\n** TODO %?")
        ("i" "Idea" entry
         (file+headline "~/Nextcloud/Notes/ideas.org" "Ideas")
         "** IDEA %^{Idea}\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:END:\n%?")
        ("c" "Contact" entry
         (file+headline "~/Nextcloud/Notes/contacts.org" "Inbox")
         "* %^{Name}
...
%?")
        ("n" "Note" entry
         (file+headline "~/Nextcloud/Notes/notes.org" "Inbox")
         "* [%<%Y-%m-%d %a>] %^{Title}\n:PROPERTIES:\n:CREATED: %U\n:CAPTURED: %a\n:END:\n%?"
         :prepend t)))
...
* Hledger
Doom Emacs, or Emacs, I don't know, use ledger by default. Use the following to make Doom Emacs use hledger instead.

The following block was to make Emacs use hledger instead of ledger, I think it is not very useful at all actually. So I'll try commenting it out and try hledger-mode instead. So if that works out, I will come back and delete this.
#+begin_src emacs-lisp
(after! ledger-mode
  (setq ledger-binary-path "hledger"
        ledger-mode-should-use-period-as-decimal-point t
        ledger-report-links-in-register nil
        ledger-default-date-format "%Y-%m-%d"))
#+end_src

In the following block, we are actually configuring hledger-mode (already installed via packages.el)

** Update
Ugh, turns out hledger-mode is mostly abandoned, while ledger-mode is actively mantained, the latest update being only 6 months ago.
So now I will comment out hledger-mode and try to just familiarize myself with ledger-mode
#+begin_src emacs-lisp
;;; Basic configuration
;; (use-package! hledger-mode
;;   :mode ("\\.journal\\'". hledger-mode)
;;   :config
;;   (setq hledger-jfile "~/Nextcloud/Notes/finances.journal")
...
No, most likely I will just remove all this shit later.
#+begin_src emacs-lisp
;; (use-package! emms
;;   :init
